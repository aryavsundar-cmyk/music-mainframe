...
| `/compare` | structure | live (Sprint 30) |
...
Up to six companies side by side, on the figures they reported. `utils/compare.js` holds three rules the table cannot break: money is **never** ranked across currencies (nothing on record converts €988.8M and $1.31B at the right date), ratios are unit-free and so always compare, and a comparison states when fiscal years end on different dates instead of lining the columns up anyway. The top of the table rebases revenue to 100 over a window **every** company reports — one sparkline each on one scale, rather than six coloured lines — and names anyone with too few consecutive years to join. A CAGR needs three consecutive years in one currency or it is not shown; a missing figure is a dash, never a zero; a balance whose tag stopped years ago (KKR last tagged long-term debt in 2021) is marked "last tagged…", never ranked, and never used in a ratio; a count of what this app happens to hold (deals on record) is not ranked at all. Segment figures say whose accounts they sit inside. Pick companies by search or from a ready-made set (the three majors, listed streaming, live and ticketing, listed rights owners), or open any company page and use *Compare with…*; the picks live in the URL, and the page exports with its caveats and the comparison limit attached. `npm run test:compare` holds every rule above.
...
**Five years, and cash as well as profit.** For each SEC filer the file also holds up to five fiscal years of every duration figure, and operating cash flow and capital expenditure (annual only — 10-Q cash-flow statements are year-to-date). Years are read from annual reports only: WMG's FY2021 was last repeated in a 2024 10-Q comparative, and taking the latest filing per period across all forms dropped it. The winning tag supplies every year it has and older years come from other candidate tags; a year is never given two values, never borrowed from a neighbour, and a missing year stays a dash. `financialConcepts.js` derives free cash flow (operating cash flow less capex, only for a year where both are filed in one currency — Sony last tagged capex for FY2021, so it has none) and operating margin. Company pages show a **Five-year record** under the latest-year table, and a figure whose latest period is not the headline year carries its own period in red rather than sitting under the wrong column.

**Company pages use the width.** `/entities/:id` runs to the wide measure (1920px): a headline strip (freshest revenue, latest quarter with its change, operating margin, free cash flow, operating metrics, deals on record), then financials; five-forces exposure, the PEPI lens and connections on record side by side; profile beside the corporate hierarchy and flows; news beside the company's own SEC filings (`/api/filings`, material forms first, routine ones a click away); related transactions. Operator notes, sources, Intelligence Hub links and a link to the entity map sit in a sticky right rail.
...
**Custom ranges back to 2018.** Beside the presets, *Custom* takes any range from 1 January 2018 — the year of the Music Modernization Act — to today, with quick picks (*Since the MMA*, *Since 2020*, *Year to date*, *Since the archive began*). Buckets size themselves to the span (days, weeks, months, quarters) and can be overridden (month, quarter, year) within limits that keep a trend readable: never more than 400 bars, never a single one. Out-of-bounds dates are clamped with a note; a start after its end is an error, never a silent swap.

The view is deliberate about evidence. News only exists from the day the archive began, so a range reaching further back **includes the 61 deals on record automatically** — they are the only evidence that old — and says so ("· auto"), with one click to leave them out and one to reset. Deals are not news items: search and entity filters apply to them (as text, and as party or headline subject); topic, source, kind and entity-type filters cannot, so when one is set the deals are left out and the view says why.

**Milestones.** Twenty-two dated, sourced market milestones (`data/milestones.js`, 2018 on) — the MMA, the MLC's blanket licence, the SAMR order on Tencent, the Eras Tour presale, DOJ v. Live Nation and the jury verdict, the label suits against Suno and Udio and the settlements after, the ELVIS Act, the EU AI Act, the FTC fee rule — each verified against a primary or major-outlet source on 22 Sep 2026, with the check's corrections kept as notes. Their forces are not assigned by hand: the engine reads each milestone's own words like any event. They are context, not activity — marked ● on the trend lines and listed, never counted.

**A reading of the range.** A few sentences computed from the counts: which force the most items tie to (with its denominator), when it peaked, what the archived news adds, how many items push against their thesis, which milestones fall in the range. Nothing asserts more than its numbers — below five items it says there is no pattern to read — and it compares like with like: when the archive begins inside the range, "busiest" is read from deals on record (the one source consistent across the span) and the jump where the archive begins is called what it is, coverage rather than a surge.
...
What changes in the work edition beyond exclusion: prospecting keeps coverage, targets, triggers and the pipeline, but message drafting is replaced with an explanation, and fit is rescored on public signals — tier, size band, role breadth and transaction cadence, weighted by `PUBLIC_FIT_WEIGHT` (1.5) so the overlay's weight moves across rather than vanishing. The glossary ships as its own Reference section, because it explains public concepts and gives away no position.
...
If SEC refuses the request — it returns 403 to anonymous callers and blocks some hosting providers — the connector falls back to the CIKs seeded in `server/sources.json`, and the UI states the reason and the fix rather than quietly scoring lower. Set `SEC_USER_AGENT` (for example `Your Name your@email`) in the environment to be recognised.
