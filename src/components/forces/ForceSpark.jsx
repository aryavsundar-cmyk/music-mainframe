...
export function ForceSpark({ series, height = 28, label }) {
  ...
  const mark = (w) => (w.milestones?.length ? ` · milestone: ${w.milestones.join('; ')}` : '')
  const tip = (w) => (w.label
    ? `${w.label}: ${w.deals ? `${w.count} (${w.deals} deal${w.deals === 1 ? '' : 's'}, ${w.events} event${w.events === 1 ? '' : 's'})` : `${w.count} event${w.count === 1 ? '' : 's'}`}${w.covered ? '' : ' — before the archive began, so news events are missing'}${mark(w)}`
    ...
    <div className="flex items-end gap-2" role="img" aria-label={`${label}: activity over ${series.length} ${series[0]?.label ? 'periods' : 'weeks'}, peak ${top}`}>
      <div className="flex items-end gap-[2px] flex-1" style={{ height }}>
        {series.map((w) => {
          ...
          return (
            <span key={w.start} title={tip(w)} className="relative flex-1 flex items-end h-full cursor-default rounded-t-[2px]" style={band}>
              {/* A market milestone in this bucket: a dot above the bar, named in the tooltip. Context, not a count. */}
              {w.milestones?.length > 0 && <span className="absolute -top-2 left-1/2 -translate-x-1/2 w-[5px] h-[5px] rounded-full bg-ink-1" aria-hidden="true" />}
