...
/** Consulting overlay rail for entity pages: categories the entity sits in + up to three engagement hypotheses. */
export function PepiLens({ entityId }) {
  const { categories, hypotheses } = getConsultingContext(entityId)
  if (categories.length === 0) return null
  return (
    <Card pad="md" tone="accent">
      <div className="t-eyebrow text-accent mb-3">{OVERLAY_LABEL}</div>
