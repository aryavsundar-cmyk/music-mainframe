...
Status: Sprints 13–16 shipped 2026-09-16 (scoring engine, coverage and target views, LinkedIn and email drafts, local records). All four phases shipped, plus Sprint 17: enrichment connectors and outcome tracking. Owner: practice lead. Drafted 2026-09-15, after Sprint 12.
...
| 15 | Re-prioritised by the operator: app-wide contrast fix and the lab glossary shipped instead. Personas, playbooks and the composer landed early in Sprint 13; the three exports move to Sprint 16 | A target list, account brief, and outreach sequence export to Word and Gamma |
| 16 | ✅ Shipped: the three exports, plus two market modules the operator added — a demand-side catalog scan and a sell-side buyer match, both built only from sourced records | Target list, account brief and outreach sequence export to Word and Gamma; the scan and the shortlist do the same. Sprint 17 added SEC filings enrichment, outcome tracking feeding back into scoring, and the two limits stated on every screen and export |
